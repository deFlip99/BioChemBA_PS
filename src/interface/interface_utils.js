//=======================================================
//Functions for a Modular approach to the DOM structure 
//=======================================================


export function createFlexContainer(id, options = {}) {
    const {
        flexDirection = "row",
        justify = "flex-start",
        alignContent = "center",
...
export function createGridContainer(id, options = {}) {
    const {
        columns = "1fr",
        rows = "auto",
        gap = "8px",
...
export function createDropdown(id, labelText, options, config = {}) {
    const {
        selectedValue = "",
        width = "100%",
        styleOverrides = ""
...
export function createContextMenu(id, items) {
...
export function createButton(id, text, options = {}) {
    const {
        variant = "primary",
        size = "medium",
        width = "auto",
    ...
    const colors = {
        primary: { bg: "#0f75b5", hover: "#128edb", text: "white" },
        secondary: { bg: "#ffffff", hover: "#128edb", text: "black" }
    };
...
export function createToggleButton(id, text, options = {}){
    const {
        size = "small",
        width = "auto",
        initialState = false,
...
export function createNavbar(id, options = {}) {
    const {
        height = "30px",
        background = "#ffffff",
        borderBottom = "1.5px solid #000000",
        padding = "0 16px",
        title = "Biochemical Visualization" 
    } = options;
    ...
    actionsContainer.setAttribute("style", 
        `display: flex; ` +
        `gap: 12px; ` +
        `align-items: center;`
    );
...
export function createBaseLayout(id, options = {}) {
    const {
...
export function createSecondaryLayout(leftId, rightId, options = {}) {
    const {
        leftWidth = "80%",
        rightWidth = "20%",
        gap = "0px"
...
export function createControlsContainer(id, options = {}) {
    const {
        width = "100%",
        height = "auto",
        padding = "16px",
...
export function createSystemContainer(systems){
    ...
    systemsList.setAttribute("style", 
        `width: 100%; ` +
        `height: auto; ` +
        `margin: 0;` +
        `overflow-y: auto; ` +
